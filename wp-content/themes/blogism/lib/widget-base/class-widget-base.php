 ...
 * @since 1.0.0
 */
class Blogism_Widget_Base extends WP_Widget {
		...
		switch ( $field_type ) {
			...
			case 'image':
				...
				<div>
					...
					<!-- <br /> -->
					<input type="button" class="select-img button button-primary" value="<?php esc_attr_e( 'Upload', 'blogism' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'blogism' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'blogism' ); ?>" />
					<?php
					$image_status = false;
					if ( ! empty( $value ) ) {
						$image_status = true;
					...
					?>
					<input type="button" value="<?php echo _x( 'X', 'Remove Button', 'blogism' ); ?>" class="button button-secondary btn-image-remove" style="<?php echo esc_attr( $remove_button_style ); ?>" />
			...
			case 'dropdown-taxonomies':
				...
				$args['multiple']        = ( isset( $field['multiple'] ) && true === $field['multiple'] ) ? true : false ;
				$args['show_option_all'] = ( isset( $field['show_option_all'] ) ) ? $field['show_option_all'] : '';
				$args['class']           = ( isset( $field['class'] ) ) ? esc_attr( $field['class'] ) : '' ;
				$qargs[] = array();
				$qargs['taxonomy'] = $args['taxonomy'];
				$value = (array) $value;
				$field_name = ( true === $args['multiple'] ) ? $this->get_field_name( $key ) . '[]' : $this->get_field_name( $key ) ;
				?>
                <p>
                    <label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>">
	                    <span class="field-label"><strong><?php echo esc_html( $field['label'] ); ?></strong></span>
                    ...
                    <?php
                    	$taxonomies = get_terms( $args['taxonomy'], $qargs );
                    ?>
                    <select
                    	name="<?php echo esc_attr( $field_name ); ?>"
                    	class="<?php echo esc_attr( $args['class'] ); ?>"
                    	id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"
                    	<?php if( true === $args['multiple'] ) { echo ' multiple="multiple"'; } ?>
                    	>
	                    <option value=""><?php echo esc_html( $args['show_option_all'] ); ?></option>
	                    <?php if ( ! is_wp_error( $taxonomies) && ! empty( $taxonomies ) ) : ?>
	                    	<?php foreach ( $taxonomies as $tax_key => $tax ) : ?>
	                    		<option value="<?php echo esc_attr( $tax->term_id ); ?>" <?php if( in_array( $tax->term_id, $value, true ) ) { echo ' selected="selected" '; } ?>><?php echo esc_html( $tax->name ); ?></option>
	                    	<?php endforeach; ?>
	                    <?php endif; ?>
                    </select>
                    <?php $this->render_description( $field, $this->get_field_id( $key ) ); ?>
